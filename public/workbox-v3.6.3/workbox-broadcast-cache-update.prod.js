this.workbox = this.workbox || {}, this.workbox.broadcastUpdate = function (t) {
    "use strict";
    try {
    ...
    const e = (t, e, s) => {
        return !s.some(s => t.headers.has(s) && e.headers.has(s)) || s.every(s => {
            const a = t.headers.has(s) === e.headers.has(s), n = t.headers.get(s) === e.headers.get(s);
            return a && n
        })
    };
    var s = {CACHE_UPDATED: "CACHE_UPDATED"};
    const a = (t, e, a, n) => {
        "BroadcastChannel" in self && t && t.postMessage({
    ...
    return t.BroadcastCacheUpdate = n, t.Plugin = class {
        constructor(t, e) {
            this.c = new n(t, e)
        }
