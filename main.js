...
(function() {
  const tittle = "Tworzę strony internetowe";
  let tittleContainer = document.querySelector(
    ".header__tittle>div>span:nth-of-type(1)"
  );
  const cursor = document.querySelector(
    ".header__tittle>div>span:nth-of-type(2)"
  );
  let number = 0;
  let index = 0;

  const writing = setInterval(() => {
    if (number < tittle.length) {
      number++;
      tittleContainer.textContent = tittleContainer.textContent + tittle[index];
      index++;
    }
  }, 100);
